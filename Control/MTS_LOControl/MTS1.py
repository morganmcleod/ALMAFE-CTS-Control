...
class LOControl(LOControl_interface):
    ...
    def __init__(self,
            ...
            polarization: int = 0,
            currentSourceResource: str = "GPIB0::25::INSTR",
    ...
    def setFrequency(self, 
            freqGHz: float, 
            refSynthAmplitude: float = 10,  # dBm
        ...
        if not self.refSynth.setAmplitude(refSynthAmplitude):
            return False, "LOControl.setFrequency: error setting synthesizer amplitude"
        ...
        if wcaFreq == 0:
            wcaFreq, ytoFreq, ytoCourse = self.loDevice.setLOFrequency(freqGHz)
            if wcaFreq == 0:
                return False, "LOControl.setFrequency: error in LODevice.setLOFrequency()"
            else:
                self.loDevice.setNullLoopIntegrator(True)
                return True, "LOControl.setFrequency: LO is tuned but not locked."
        else:
            return True, ""
